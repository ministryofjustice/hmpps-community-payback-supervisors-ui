  ...
  describe('isValidTime', () => {
    ...
    describe('isAfterTime', () => {
      it.each([
        ...
        expect(DateTimeFormats.isAfterTime(time, timeToCompare)).toBe(false)
      })
      ...
      it.each([
        ...
        ['11:00:00', '10:10:00'],
        ['11:00:01', '10:10:01'],
      ])('returns true if time is before time to compare', (time: string, timeToCompare: string) => {
        expect(DateTimeFormats.isAfterTime(time, timeToCompare)).toBe(true)
      })

      it('returns false if time is same as time to compare', () => {
        expect(DateTimeFormats.isAfterTime('11:00', '11:00')).toBe(false)
      })
    })
  })
})
